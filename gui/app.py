...
class App:
    ...
    def button(self, text: str, x: int, y: int, width: int, height: int, inactive_color: Color, active_color: Color, action: Optional[Callable[[], None]]):
        ...
        if self.piece is not None and self.piece.position in self.board.board:
            ...
            if self.player_side == self.board.moving_side:
                ...
                for move in all_moves:
                    pygame.draw.circle(self.window, RED, self.get_piece_position(move.move_squares[len(self.moves)]), square_size // 20)
    ...
    def draw_board(self):
        ...
        for row in range(8):
            for col in range(row % 2, 8, 2):
                pygame.draw.rect(self.window, LIGHT_BROWN, (row*square_size, col*square_size + timer_height, square_size, square_size))
        self.draw_pieces()
    ...
    def main_menu(self):
        ...
        self.button("SINGLE PLAYER", int(width/4), 100, int(width/2), 100, BLACK, RED, self.get_update_screen_action(self.singleplayer_menu))
    ...
    def multiplayer_menu(self):
        ...
        self.window.fill(LIGHT_BROWN)
        self.button("HOST", int(width/4), 100, int(width/2), 100, BLACK, RED, self.get_update_screen_action(self.host_menu))
    ...
    def singleplayer_menu(self):
        ...
        self.button("PLAY", int(width/4), 350, int(width/2), 100, DARK_BROWN, BLACK, self.get_update_screen_action(self.singleplayer_game))
    ...
    def perform_player_action(self, mouse: Any, is_multiplayer: bool=False):
        ...
        if (x, y) in self.board.board.keys():
            ...
            if len(self.board.generate_moves()[0]):
                all_moves = captures
            if (x, y) in map(lambda m: m.move_squares[len(self.moves)], filter(lambda m: len(m.move_squares) > len(self.moves), all_moves)):
